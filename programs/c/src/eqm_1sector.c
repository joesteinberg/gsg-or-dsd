...
void set_neqm()
{
  ...
  neqm = (NT+1)*NC + 2*(NT+1)*NC + (NT+1)*NC + NT*NC + NT*(NC-1) + NT;
  if(f_adj_cost)
    {
      neqm = neqm + NT*NC*NC + NT*NC;
    }
  if(m_adj_cost)
    {
      neqm = neqm + NT*NC*NC + NT*NC;
...
void init_vars(eqm * e)
{
  ...
  SET_ALL_V(e->py_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->va_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->md_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->k_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->l_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->is_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->lp_t,(NT+1)*NC,0.0);

  SET_ALL_V(e->nxsf_t,(NT+1)*NC*NC,0.0);
  SET_ALL_V(e->nxsm_t,(NT+1)*NC*NC,0.0);

  SET_ALL_V(e->pm_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->m_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->m2_t,(NT+1)*NC*NC,0.0);

  SET_ALL_V(e->p_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->q_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->q2_t,(NT+1)*NC*NC,0.0);

  SET_ALL_V(e->c_t,(NT+1)*NC,0.0);
  SET_ALL_V(e->i_t,(NT+1)*NC,0.0);
}
...
uint stack_bgp_vars(double * myx, const eqm * e)
{
  ...
  COPY_SUBVECTOR_LOG(myx+nx,e->l_t[t],NC);
  nx=nx+NC;

  COPY_SUBVECTOR_LOG(myx+nx,e->py_t[t],NC);
  nx=nx+NC;

  COPY_SUBVECTOR_LOG(myx+nx,e->c_t[t],NC);
  nx=nx+NC;
...
uint unstack_bgp_vars(eqm * e, const double * myx)
{
  ...
  COPY_SUBVECTOR_EXP(e->l_t[t],myx+nx,NC);
  nx=nx+NC;

  COPY_SUBVECTOR_EXP(e->py_t[t],myx+nx,NC);
  nx=nx+NC;

  COPY_SUBVECTOR_EXP(e->c_t[t],myx+nx,NC);
  nx=nx+NC;
...
uint stack_eqm_vars(double * myx, const eqm * e)
{
  ...
  COPY_SUBVECTOR_LOG(myx+nx,&(e->is_t[t0]),(nn-1)*NC);
  nx = nx + (nn-1)*NC;

  COPY_SUBVECTOR_LOG(myx+nx,&(e->l_t[t0]),(nn)*NC);
  nx = nx + (nn)*NC;

  COPY_SUBVECTOR_LOG(myx+nx,&(e->py_t[t0]),(nn)*NC);
  nx = nx + (nn)*NC;

  COPY_SUBVECTOR_LOG(myx+nx,&(e->c_t[t0]),(nn)*NC);
  nx = nx + (nn)*NC;
  ...
  if(f_adj_cost)
    {
      COPY_SUBVECTOR_LOG(myx+nx,&(e->q2_t[t0]),(nn-1)*NC*NC);
      nx = nx + (nn-1)*NC*NC;

      COPY_SUBVECTOR_LOG(myx+nx,&(e->p_t[t0]),(nn-1)*NC);
      nx = nx + (nn-1)*NC;
    }
  ...
  if(m_adj_cost)
    {
      if(noio_flag==0)
	{
	  COPY_SUBVECTOR_LOG(myx+nx,&(e->m2_t[t0]),(nn-1)*NC*NC);
	  nx = nx + (nn-1)*NC*NC;

	  COPY_SUBVECTOR_LOG(myx+nx,&(e->pm_t[t0]),(nn-1)*NC);
	  nx = nx + (nn-1)*NC;
	}
      else
	{
	  COPY_SUBVECTOR(myx+nx,&(e->m2_t[t0]),(nn-1)*NC*NC);
	  nx = nx + (nn-1)*NC*NC;

	  COPY_SUBVECTOR(myx+nx,&(e->pm_t[t0]),(nn-1)*NC);
	  nx = nx + (nn-1)*NC;
	}
    }
...
uint unstack_eqm_vars(eqm * e, const double * myx)
{
  ...
  COPY_SUBVECTOR_EXP(&(e->is_t[t0]),myx+nx,(nn-1)*NC);
  nx = nx + (nn-1)*NC;

  COPY_SUBVECTOR_EXP(&(e->l_t[t0]),myx+nx,(nn)*NC);
  nx = nx + (nn)*NC;

  COPY_SUBVECTOR_EXP(&(e->py_t[t0]),myx+nx,(nn)*NC);
  nx = nx + (nn)*NC;

  COPY_SUBVECTOR_EXP(&(e->c_t[t0]),myx+nx,(nn)*NC);
  nx = nx + (nn)*NC;
  ...
  if(f_adj_cost)
    {
      COPY_SUBVECTOR_EXP(&(e->q2_t[t0]),myx+nx,(nn-1)*NC*NC);
      nx = nx + (nn-1)*NC*NC;

      COPY_SUBVECTOR_EXP(&(e->p_t[t0]),myx+nx,(nn-1)*NC);
      nx = nx + (nn-1)*NC;
    }
  ...
  if(m_adj_cost)
    {
      if(noio_flag==0)
	{
	  COPY_SUBVECTOR_EXP(&(e->m2_t[t0]),myx+nx,(nn-1)*NC*NC);
	  nx = nx + (nn-1)*NC*NC;
	  
	  COPY_SUBVECTOR_EXP(&(e->pm_t[t0]),myx+nx,(nn-1)*NC);
	  nx = nx + (nn-1)*NC;
	}
      else
	{
	  COPY_SUBVECTOR(&(e->m2_t[t0]),myx+nx,(nn-1)*NC*NC);
	  nx = nx + (nn-1)*NC*NC;
	  
	  COPY_SUBVECTOR(&(e->pm_t[t0]),myx+nx,(nn-1)*NC);
	  nx = nx + (nn-1)*NC;
...
uint set_initial_bgp_guess()
{
  ...
  for(i=0; i<NC; i++)
    {
      e->w_t[t][i] = 1.0;
      e->c_t[t][i] = p->c0[i] * p->a_ts[t][i] * p->lbar_ts[t][i]/p->lbar0[i];
      e->l_t[t][i] = p->l0[i] * p->lbar_ts[t][i]/p->lbar0[i];
      e->k_t[t][i] = p->k0[i] * p->a_ts[t][i] * p->lbar_ts[t][i]/p->lbar0[i];
      e->py_t[t][i] = 1.0;
    }
...
uint set_initial_eqm_guess()
{
  ...
  LOGSPACE_2D(p->l0,e->l_t[NT],NT+1,NC,e->l_t);
  LOGSPACE_2D(p->c0,e->c_t[NT],NT+1,NC,e->c_t);
  LOGSPACE_2D(p->q0,e->q_t[NT],NT+1,NC,e->q_t);
  LOGSPACE_2D(p->m0,e->m_t[NT],NT+1,NC,e->m_t);
  LINSPACE_2D(tmpp,e->w_t[NT],NT+1,NC,e->w_t);
  LINSPACE_2D(tmpp2,e->py_t[NT],NT+1,NC,e->py_t);
  LINSPACE_2D(tmpp3,e->p_t[NT],NT+1,NC,e->p_t);
  LINSPACE_2D(tmpp3,e->pm_t[NT],NT+1,NC,e->pm_t);

  for(i=0; i<NC; i++)
    {
      for(t=0; t<NT; t++)
	{
	  e->is_t[t][i] = e->k_t[t+1][i] - (1.0-p->delta)*e->k_t[t][i];
	}
    }
  ...
  for(t=0; t<(NT+1); t++)
      ...
      for(i=0; i<NC; i++)
	{
	  for(j=0; j<NC; j++)
	    {
	      e->q2_t[t][i][j] = (p->q02[i][j]/p->q0[i]) * e->q_t[t][i];
	      if(noio_flag)
		{
		  e->m2_t[t][i][j] = 0.0;
		}
		...
		{
		  e->m2_t[t][i][j] = (p->m02[i][j]/p->m0[i]) * e->m_t[t][i];
		}
	    }      
	}
    }
  ...
  if(file)
    {
      ...
      fprintf(file,",y,va,i,l");
      for(j=0; j<NC; j++)
	{
	  if(j!=i)
	    {
	      //fprintf(file,",rer%d,tb%d",j,j);
	      fprintf(file,",tbsm-%d,tbsf-%d",j,j);
	    }
	}
      fprintf(file,"\n");
      ...
      for(t=0;t<(NT+1);t++)
	{
	  ...
	  fprintf(file,"%0.16f,",100.0*e->p_t[t][i]);
	  ...
	  fprintf(file,",%0.16f",e->va_t[t][i]);
	  fprintf(file,",%0.16f",e->is_t[t][i]);
	  fprintf(file,",%0.16f",e->l_t[t][i]);
	  for(j=0; j<NC; j++)
	    {
	      if(j!=i)
		{
		  ...
		  fprintf(file,",%0.16f",100.0*e->nxsm_t[t][i][j]/e->ngdp_t[t][i]);
		  fprintf(file,",%0.16f",100.0*e->nxsf_t[t][i][j]/e->ngdp_t[t][i]);
		}
	    }
	  fprintf(file,"\n");
	}
...
void set_vars(eqm * e, const params * p, uint t, uint bgp)
{
  ...
  SET_ALL_V(e->nxsm_t[t],NC*NC,0.0);
  SET_ALL_V(e->nxsf_t[t],NC*NC,0.0);

  for(i=0; i<NC; i++)
    {
      e->va_t[t][i] = e->tau_a_ts[t][i] * prod_va(e->k_t[t][i],e->l_t[t][i],p->A[i],p->alpha[i],p->a_ts[t][i]);
      e->y_t[t][i] = e->va_t[t][i]/p->lam_va[i];
      e->ngdp_t[t][i] = e->ngdp_t[t][i] + e->py_t[t][i]*e->y_t[t][i];
      e->rgdp_t[t][i] = e->rgdp_t[t][i] + e->y_t[t][i];
      e->lp_t[t][i] = e->va_t[t][i]/e->l_t[t][i];
      e->md_t[t][i] = e->y_t[t][i]*p->lam[i];
      e->ll_t[t][i] = e->l_t[t][i];
      ...
      if(f_adj_cost && t<NT)
	{
	  for(j=0; j<NC; j++)
	      ...
	      if(j!=i)
		{
		  ...
		  if(t>0)
		    {
		      tmp = e->q2_t[t][i][j]/e->q2_t[t-1][i][j] - 1.0;
		    }
		  else
		    {
		      tmp = e->q2_t[t][i][j]/p->q02[i][j] - 1.0;
		    }
      ...
      if(m_adj_cost && t<NT && noio_flag==0)
	{
	  for(j=0; j<NC; j++)
	      ...
	      if(j!=i)
		{
		  ...
		  if(t>0)
		    {
		      tmp = e->m2_t[t][i][j]/e->m2_t[t-1][i][j] - 1.0;
		    }
		  else
		    {
		      tmp = e->m2_t[t][i][j]/p->m02[i][j] - 1.0;
		    }
		  e->ll_t[t][i] = e->ll_t[t][i] + 
		    (p->etaM/2.0) * tmp * tmp;
		}
      ...
      if(t<NT)
	{
	  if(t==(NT-1) || k_adj_cost==0)
	    {
	      e->k_t[t+1][i] = (1.0-p->delta) * e->k_t[t][i] + e->is_t[t][i];
	    }
	  else
	    {
	      e->k_t[t+1][i] = (1.0-p->delta) * e->k_t[t][i] + 
		phiK(e->is_t[t][i]/e->k_t[t][i],p->delta,p->ga_bgp,p->etaK) * e->k_t[t][i];
	    }
	}
      else
	{
	  e->is_t[t][i] = (p->ga_bgp-1.0+p->delta) * e->k_t[t][i];
	}
      
      e->kk_t[t][i] = 0.0;
      e->ii_t[t][i] = 0.0;
      e->kk_t[t][i] = e->kk_t[t][i] + e->k_t[t][i];
      e->ii_t[t][i] = e->ii_t[t][i] + e->is_t[t][i];
      ...
      if(!m_adj_cost || t==NT)
	{
	  ...
	  if(noio_flag==0)
	      ...
	      for(j=0; j<NC; j++)
		{
		  ...
		  e->pm_t[t][i] = e->pm_t[t][i] + 
		    pow(p->mu[i][j],1.0/(1.0-p->zeta[i])) * 
		    pow(tmp*e->py_t[t][j],p->zeta[i]/(p->zeta[i]-1.0));		      
		}
	      e->pm_t[t][i] = (1.0/p->M[i]) * 
		pow(e->pm_t[t][i],(p->zeta[i]-1.0)/p->zeta[i]);
	    }
	}
      ...
      if(!f_adj_cost || t==NT)
	{
	  ...
	  for(j=0; j<NC; j++)
	      ...
	      if(j!=i)
		{
	      ...
	      e->p_t[t][i] = e->p_t[t][i] + 
		pow(p->theta[i][j],1.0/(1.0-p->sig[i])) * 
		pow(tmp*e->py_t[t][j],p->sig[i]/(p->sig[i]-1.0));
	    }
	  e->p_t[t][i] = (1.0/p->H[i]) * 
	    pow(e->p_t[t][i],(p->sig[i]-1.0)/p->sig[i]);
	}
      ...
      if(t>0)
	{
	  ...
	  e->Q_t[t-1][i] = p->beta[i] * (mutp / e->p_t[t][i]) / (mut / e->p_t[t-1][i]);
	}
      if(t==NT)
	{
	  e->Q_t[t][i] = p->beta[i];
      ...
      e->pi_t[t][i] = 1.0;
      e->pi_t[t][i] = e->pi_t[t][i] * e->p_t[t][i];
      e->i_t[t][i] = e->pi_t[t][i] * e->ii_t[t][i]/e->p_t[t][i];

      e->q_t[t][i] = e->c_t[t][i] + e->i_t[t][i];
      e->m_t[t][i] = 0.0;
      e->m_t[t][i] = e->m_t[t][i] + e->md_t[t][i];
      ...
      if(!m_adj_cost || t==NT)
	{
	  for(j=0; j<NC; j++)
		...
		{
		  e->m2_t[t][i][j] = e->m_t[t][i] * 
		    pow(tmp*e->py_t[t][j],1.0/(p->zeta[i]-1.0)) * 
		    pow(e->pm_t[t][i]*p->mu[i][j]*pow(p->M[i],p->zeta[i]),1.0/(1.0-p->zeta[i]));
		}
	    }
	}
      ...
      if(!f_adj_cost || t==NT)
	{
	  for(j=0; j<NC; j++)
	      ...
	      if(j!=i)
		{
	      ...
	      e->q2_t[t][i][j] = e->q_t[t][i] * 
		pow(tmp*e->py_t[t][j],1.0/(p->sig[i]-1.0)) * 
		pow(e->p_t[t][i]*p->theta[i][j]*pow(p->H[i],p->sig[i]),1.0/(1.0-p->sig[i]));
	    }
	}

      e->cpi_t[t][i] = e->p_t[t][i];
      e->cc_t[t][i] = e->c_t[t][i];
  ...
  for(i=0; i<NC; i++)
    {
      for(j=0; j<NC; j++)
	{
	  e->ngdp_t[t][i] = e->ngdp_t[t][i] - e->py_t[t][j]*e->m2_t[t][i][j];
	  e->rgdp_t[t][i] = e->rgdp_t[t][i] - e->m2_t[t][i][j];

	  if(j!=i)
	    {
	      e->rer_t[t][i][j] = 100*e->cpi_t[t][j]/e->cpi_t[t][i];

	      double m = e->py_t[t][i]*e->m2_t[t][j][i];
	      double f = e->py_t[t][i]*e->q2_t[t][j][i];
	
	      e->nxsm_t[t][i][j] = m;
	      e->nxsf_t[t][i][j] = f;
	      e->ex_t[t][i][j] = e->ex_t[t][i][j] + m + f;

	      m = e->py_t[t][j]*e->m2_t[t][i][j];
	      f = e->py_t[t][j]*e->q2_t[t][i][j];

	      e->nxsm_t[t][i][j] = e->nxsm_t[t][i][j] - m;
	      e->nxsf_t[t][i][j] = e->nxsf_t[t][i][j] - f;
	      e->im_t[t][i][j] = e->im_t[t][i][j] + m + f;

	      e->nx_t[t][i][j] = e->nx_t[t][i][j] + e->nxsm_t[t][i][j] + e->nxsf_t[t][i][j];
      ...
      e->cy_t[t][i] = 100.0*(e->p_t[t][i]*e->c_t[t][i])/e->ngdp_t[t][i];
...
uint eval_bgp_conds(const double * myx, double * myf, uint tn)
{
  ...
  for(i=0; i<NC; i++)
    {
      myf[nx] = mpk_rk(p,e,t,i);
      nx=nx+1;

      myf[nx] = mpl_w(p,e,t,i);
      nx=nx+1;

      myf[nx] = mkt_clear_y(p,e,t,i);
      nx=nx+1;
...
uint eval_eqm_conds(const double * myx, double * myf, uint tn)
{
  ...
  for(t=t0; t<(NT+1); t++)
      ...
      for(i=0; i<NC; i++)
	{
	  ...
	  if(m_adj_cost && t<NT)
	    {
	      myf[nx] = prod_m_chk(p,e,t,i);
	      nx = nx+1;

	      uint j;
	      for(j=0; j<NC; j++)
		{
		  myf[nx] = foc_m2(p,e,t,i,j);
		  nx = nx+1;
		}
	    }
	  ...
	  if(f_adj_cost && t<NT)
	    {
	      myf[nx] = prod_q_chk(p,e,t,i);
	      nx = nx+1;

	      uint j;
	      for(j=0; j<NC; j++)
		{
		  myf[nx] = foc_q2(p,e,t,i,j);
		  nx = nx+1;
		}
	    }
	}
