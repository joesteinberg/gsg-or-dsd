...
uint parse_args(int argc, char **argv)
{
  ...
  while((opt = getopt(argc, argv, "tdgjsxnerazwqb")) != -1){
    switch(opt){
    case 'n':
      noio_flag=1;
      cnt++;
    ...
    case 'x':
      ...
      break;
    case '?':
      fprintf(logfile,"\nIncorrect command line option: %c. Possible options: -t -d -g -j -s -n -e -r -a -z -b\n",opt);
      ...
      fprintf(logfile,"\t-j: no RW investment wedge (and no RW investment targeting)\n");
      fprintf(logfile,"\t-s: only saving wedges, match RIR\n");
      fprintf(logfile,"\t-x: only saving wedges, match US investment\n");
      fprintf(logfile,"\t-n: no input-output linkages\n");
      fprintf(logfile,"\t-e: high etaK\n");
      fprintf(logfile,"\t-r: low etaK\n");
      fprintf(logfile,"\t-a: higher Armingtons\n");
      fprintf(logfile,"\t-z: lower Armingtons\n");
      fprintf(logfile,"\t-w: higher wedge persistence\n");
      fprintf(logfile,"\t-q: lower wedge persistence\n");
      fprintf(logfile,"\t-b: lower long run interest rate\n");
  ...
  if(cnt>1)
    {
      fprintf(logfile,"\nIncorrect command line option: %c. Possible options: -t -d -g -j -s -n -e -r -b\n",opt);
      fprintf(logfile,"\t-t: do not include trade wedges to match RER\n");
      fprintf(logfile,"\t-d: no demographic change\n");
      fprintf(logfile,"\t-g: constant and symmetric productivity growth\n");
      fprintf(logfile,"\t-j: no RW investment wedge (and no RW investment targeting)\n");
...
int quant_exercise()
{
  ...
  write_eqm_vars(&(eee0[0]),"vars0_2c_usa",0);
  write_eqm_vars(&(eee0[0]),"vars0_2c_row",1);
  ...
  write_eqm_vars(&(eee1[0]),"vars1_2c_usa",0);
  write_eqm_vars(&(eee1[0]),"vars1_2c_row",1);
  ...
  if(!cal)
      ...
      for(i=0; i<NC; i++)
	{
	  ...
	  snprintf(s1,15,"vars2_%d_2c_usa",cnt);
	  snprintf(s2,15,"vars2_%d_2c_row",cnt);
	  ...
	  if(!tau_s_only_flag && !tau_s_only_flag2 && (i==0 || (!no_rw_tau_i_flag && !no_rw_tau_i_flag2)))
	    {
	      fprintf(logfile, KNRM "----------------------------------------------------------------------\n" RESET);
...
#pragma omp parallel num_threads(nt)
  ...
  if(tau_t_flag==0 && !tau_s_only_flag && !tau_s_only_flag2)
    {
    ...
    {
      fprintf(logfile,KGRN "Sensitivity analysis with no RW inv. wedge (and no RW inv. targeting)\n\n\n" RESET);
    }
  else if(tau_s_only_flag)
    {
      fprintf(logfile,KGRN "Sensitivity analysis with only saving wedges (US TB and RIR only)\n\n\n" RESET);
    }
  else if(tau_s_only_flag2)
    {
      fprintf(logfile,KGRN "Sensitivity analysis with only saving wedges (US TB and inv only)\n\n\n" RESET);
    }
  else if(noio_flag==1)
    {
    ...
    {
      fprintf(logfile,KGRN "Sensitivity analysis with lower K adj costs\n\n\n" RESET);
    }
  else if(low_etaK_flag==1)
    {
      fprintf(logfile,KGRN "Sensitivity analysis with higher K adj costs\n\n\n" RESET);
    }
  else if(low_r_flag==1)
    {
      fprintf(logfile,KGRN "Sensitivity analysis with lower long run interest rate\n\n\n" RESET);
    }
  else
    {
